...
class FluidSim {
  ...
  _createPasses() {
    ...
    const efMesh = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), new THREE.RawShaderMaterial({
    ...
    const viscPass = new ShaderPass(r, face_vert, viscous_frag, {
      boundarySpace: { value: cellScale },
  ...
  update(time) {
...

    {
      ...
      const u = (this.externalForce.mesh.material)
        .uniforms;
...
export function PixelLiquidBg({
  ...
  useEffect(() => {
    ...
    const themeObserver = new MutationObserver(() => {
      ...
      writePaletteData(palette.image.data, stops);
      palette.needsUpdate = true;
      const bg = getBgColor(nowDark);
      (outputUniforms.bgColor.value).set(bg.x, bg.y, bg.z, bg.w);
    });
  ...
  return (
    <div
      ref={mountRef}
      className={cn("relative isolate w-full h-full overflow-hidden bg-background", className)}
      {...props}>
      {children && (
